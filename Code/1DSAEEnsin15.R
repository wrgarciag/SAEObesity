...
## Variables auxiliares para modelos de regresion----
...
dt_ensin[,gedad:=cut(AN_EDAD,              # Numeric input vector
                 breaks =seq(0,65,5),                  # Number or vector of breaks
                 labels =seq(0,60,5))]
...
dt_ensin <- dt_ensin[,c('LLAVE_HOGAR','LLAVE_PERSONA','obeso','DEPTO','Subregion','rural','mujer','gedad','cod_UPM_rev','Estrato_UPM','fpc_upm','fpc_USM',
                            'USM','fpc_utm','UTM',
...
#dt_ensin_aux <- dt_ensin_aux[, lapply(.SD, function(x){mean(x*FactorExpansionPer,na.rm=T)}), by="cod_UPM_rev"]
